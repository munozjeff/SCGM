...
export default function PortfolioUpdate() {
    ...
    // Columns
    const columns = ['NUMERO', 'SALDO', 'ABONO', 'FECHA_CARTERA'];
    const selectFields = []; // No fields are selects

    const [filters, setFilters] = useState({ NUMERO: '', SALDO: '', ABONO: '', FECHA_CARTERA: '' });
    ...
    const clearFilters = () => setFilters({ NUMERO: '', SALDO: '', ABONO: '', FECHA_CARTERA: '' });
    ...
    const handleNewRecord = () => {
        setEditForm({ NUMERO: '', SALDO: '', ABONO: '', FECHA_CARTERA: '' });
        setIsNewRecord(true);
        setIsEditModalOpen(true);
    };
            ...
            <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '0.3rem 0.5rem', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <h2 style={{ fontSize: '0.85rem', margin: 0, fontWeight: '600', whiteSpace: 'nowrap' }}>Cartera</h2>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button className="btn-secondary" onClick={() => downloadTemplate(['NUMERO', 'SALDO', 'ABONO', 'FECHA_CARTERA'], 'Plantilla_Cartera')} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem', whiteSpace: 'nowrap' }}>📥 Plantilla</button>
                        ...
                        <tbody>
                            {currentItems.map((item, index) => (
                                <tr key={index} onDoubleClick={() => handleEditClick(item)} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', background: index % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent' }} className="table-row-hover">
                                    ...
                                    <td style={{ padding: '0.6rem' }}>{item.SALDO}</td>
                                    <td style={{ padding: '0.6rem' }}>{item.ABONO}</td>
                                    <td style={{ padding: '0.6rem' }}>{item.FECHA_CARTERA}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {isEditModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div className="glass-panel" style={{ width: '400px', padding: '2rem' }}>
                        <h3 style={{ marginBottom: '1.5rem' }}>{isNewRecord ? 'Nuevo Registro' : 'Editar Cartera'}</h3>
                        <form onSubmit={handleSave}>
                            ...
                            <div style={{ marginBottom: '1rem' }}>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Saldo</label>
                                <input value={editForm.SALDO || ''} onChange={e => setEditForm({ ...editForm, SALDO: e.target.value })} style={{ width: '100%', padding: '0.5rem' }} />
                            </div>
                            <div style={{ marginBottom: '1rem' }}>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Abono</label>
                                <input value={editForm.ABONO || ''} onChange={e => setEditForm({ ...editForm, ABONO: e.target.value })} style={{ width: '100%', padding: '0.5rem' }} />
                            </div>
                            <div style={{ marginBottom: '1.5rem' }}>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem' }}>Fecha Cartera</label>
                                <input value={editForm.FECHA_CARTERA || ''} onChange={e => setEditForm({ ...editForm, FECHA_CARTERA: e.target.value })} style={{ width: '100%', padding: '0.5rem' }} />
                            </div>
