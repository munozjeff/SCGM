...
import { updateSalesType, getAllMonths } from '../services/SalesService';
...
export default function SalesTypeUpdate() {
    ...
    const handleSingleSubmit = async (e) => {
        ...
        setResult(null);
        try {
            const res = await updateSalesType(month, [formData]);
            setResult(res);
            if (res.updated > 0) {
                setFormData({ NUMERO: '', TIPO_VENTA: '' });
            }
        } catch (err) {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                const updates = data.map(row => {
                    ...
                    const tipo = row['TIPO VENTA'] || row['Tipo Venta'] || row['TIPO_VENTA'] || row['tipo venta'];

                    return {
                        NUMERO: String(numero || '').trim(),
                        TIPO_VENTA: tipo ? String(tipo).trim() : null
                    };
                ...
                {mode === 'single' ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '2rem', border: '2px dashed var(--glass-border)', borderRadius: 'var(--radius-md)' }}>
                        ...
                        <ul style={{ listStyle: 'none', marginBottom: '1.5rem', color: 'var(--text-muted)', textAlign: 'left', display: 'inline-block' }}>
                            ...
                            <li><code>TIPO VENTA</code> (Requerido)</li>
                        </ul>
                        <br />
                        <input
                            type="file"
