        ...
        try {
            ...
            if (currentUser) {
                ...
                const actionDetails = isNewRecord
                    ...
                    : `Actualizó registro: ${editForm.NUMERO} a Fecha: ${editForm.FECHA_ACTIVACION}`;
                logUserAction(currentUser.uid, currentUser.email, 'UPDATE_ACTIVATION', actionDetails, { month, ...res });
            }
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                if (currentUser) {
                    updateUserActivity(currentUser.uid);
                    logUserAction(currentUser.uid, currentUser.email, 'IMPORT_ACTIVATION', `Importó ${updates.length} registros`, { month, ...res });
                }
            } catch (err) { alert(err.message); }
            setLoading(false);
        };
                ...
                <div style={{ padding: '0.3rem 0.5rem', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button className="btn-secondary" onClick={() => downloadTemplate(['NUMERO', 'FECHA_ACTIVACION'], 'Plantilla_Activacion')} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem', whiteSpace: 'nowrap' }}>📥 Plantilla</button>
